...
public class LibroTerror extends Libro {
    ...
    public LibroTerror(String nombre, String autor, String editorial, LocalDate fechaPublicacion, double precio, int stock, SubgeneroTerror subgenero) {
    ...
    protected void filtrarPorID(int ID) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getId() == ID).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorFecha(LocalDate Fecha) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getFechaPublicacion() == Fecha).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorPrecio(double precio) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getPrecio() == precio).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorLetra(char letra) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getletraAutor() == letra).forEach(libro -> System.out.println(libro.toString()));
    }

    @Override
    protected void filtrarPorNombre(String nombre) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> Objects.equals(libro.getNombre(), nombre)).forEach(libro -> System.out.println(libro.toString()));
    }

    @Override
    protected void filtrarPorStock(int stock) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getStock() == stock).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorEditorial(String editorial) {
        Biblioteca.libros.get(Genero.TERROR).stream().filter(libro -> libro.getEditorial() == editorial).forEach(libro -> System.out.println(libro.toString()));
    }
}
