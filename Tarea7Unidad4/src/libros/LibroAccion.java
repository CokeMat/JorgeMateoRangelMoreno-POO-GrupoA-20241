...
public class LibroAccion extends Libro{
    ...
    public LibroAccion(String nombre, String autor, String editorial, String fechaPublicacion, double precio, int stock, SubgeneroAccion subgenero) {
    ...
    protected void filtrarPorID(int ID) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getId() == ID).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorFecha(String Fecha) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getFechaPublicacion() == Fecha).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorPrecio(double precio) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getPrecio() == precio).forEach(libro -> System.out.println(libro.toString()));
    }
    @Override
    protected void filtrarPorLetra(char letra) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getletraAutor() == letra).forEach(libro -> System.out.println(libro.toString()));
    }

    @Override
    protected void filtrarPorNombre(String nombre) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getNombre() == nombre).forEach(libro -> System.out.println(libro.toString()));
    }

    @Override
    protected void filtrarPorStock(int stock) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getStock() == stock).forEach(libro -> System.out.println(libro.toString()));
    }

    @Override
    protected void filtrarPorEditorial(String editorial) {
        Biblioteca.libros.get(Genero.ACCION.toString()).stream().filter(libro -> libro.getEditorial() == editorial).forEach(libro -> System.out.println(libro.toString()));
    }
}
